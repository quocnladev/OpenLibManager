    ...
    public partial class frmBorrower : Form
    {
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            if (config.dt.Rows.Count > 0)
            {
                ...
                config.Execute_CUD(sql, "error to execute the query", "Độc giả đã được cập nhật trong database.");
            ...
            else
            {
                ...
                config.Execute_CUD(sql, "error to execute the query", "Độc giả mới đã được thêm vào database.");
        ...
        { 
            if (MessageBox.Show("Bạn có chắc sẽ xóa độc giả này??", "Xóa", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes) { 
                sql = "UPDATE `tblborrower` SET  `Stats` =  'NotActive' WHERE `BorrowerId` ='" + dtg_ABorrowLists.CurrentRow.Cells[0].Value + "'";
                config.Execute_Query(sql);

                MessageBox.Show("Độc giả đã được xóa trong database.", "Thành công", MessageBoxButtons.OK);
                btn_New_Click(sender, e);
            } 
        }
