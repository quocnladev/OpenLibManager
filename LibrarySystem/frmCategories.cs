    ...
    public partial class frmCategories : Form
    {
        ...
        private void btnsave_Click(object sender, EventArgs e)
        {
            ...
            if (config.dt.Rows.Count > 0)
            {
                ...
                config.Execute_CUD(sql, "error to execute the query.", "Thể loại sách đã được lưu trong database.");
            }
            else
            {
                sql = "INSERT INTO `tblcategory` (`Category`,`DDecimal`) VALUES ('" + txtCategory.Text + "','" + txtDeweyDecimal.Text + "')";
                config.Execute_CUD(sql, "error to execute the query.", "Thể loại sách mới đã được lưu trong database.");
            } 
            btnnew_Click(sender, e);
